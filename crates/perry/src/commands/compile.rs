...
fn find_harmonyos_sdk() -> Option<PathBuf> {
    fn normalize(p: PathBuf) -> Option<PathBuf> {
        ...
        // DevEco 6.x layout: the SDK root contains `default/openharmony/native`
        // (a single "default" folder replaces the per-API-level numeric dir
        // from 5.x). Applies when `p` is e.g.
        // `/Applications/DevEco-Studio.app/Contents/sdk`.
        let deveco6 = p.join("default").join("openharmony").join("native");
        if deveco6.join("llvm").join("bin").exists() && deveco6.join("sysroot").exists() {
            return Some(deveco6);
        }
        // DevEco 6.x, one level deeper — `p` already points at
        // `<root>/default` or `<root>/default/openharmony`.
        let v6_inner = p.join("openharmony").join("native");
        if v6_inner.join("llvm").join("bin").exists() && v6_inner.join("sysroot").exists() {
            return Some(v6_inner);
        }
        // DevEco 5.x layout nests the API-level dir: <root>/openharmony/<api>/native
    ...
    // macOS DevEco 6.x bundles the SDK inside the app itself — no per-user
    // install location. Probe that first since it's a fixed path and every
    // 6.x user has it.
    #[cfg(target_os = "macos")]
    candidates.push(PathBuf::from("/Applications/DevEco-Studio.app/Contents/sdk"));
    if let Some(h) = home {
        // macOS DevEco 5.x default:
...
pub fn run(args: CompileArgs, format: OutputFormat, use_color: bool, verbose: u8) -> Result<CompileResult> {
    ...
    if is_harmonyos {
        let triple = rust_target_triple(target.as_deref()).unwrap_or("aarch64-unknown-linux-ohos");
        let build_roots: Vec<PathBuf> = {
        ...
        let mut native_objs: Vec<PathBuf> = Vec::new();
        ...
        if !native_objs.is_empty() && matches!(format, OutputFormat::Text) {
    ...
    if is_watchos {
        ...
        // HarmonyOS musl sysroot — no macOS frameworks (we're ELF, not Mach-O),
        // no glibc `-lssl`/`-lcrypto` (OHOS doesn't ship OpenSSL; crypto lives
        // under Huawei's own libcrypto_* / libopenssl_napi services, which
        // we'd dlopen if perry-stdlib needed them). libc/libm/libdl/libpthread
        // are all folded into musl's libc.a, so the `-l` requests are no-ops
        // but harmless.
        cmd.arg("-Wl,--allow-multiple-definition")
           .arg("-lm")
           .arg("-lpthread")
        ...
        // that ohos_napi.rs externs. OHOS's shared-object convention is
        // `<name>.z.so` — the `-l` flag strips `lib` and `.so` but NOT the
        // `.z` middle, so we spell it out.
        cmd.arg("-lace_napi.z");
    } else if is_linux {
...
<dict>
    ...
    // foreign architectures (macOS strip fails on ELF with "non-object and
    // non-archive file" — a warning only, but noisy).
    // Skip when PERRY_DEBUG_SYMBOLS=1 is set — keep symbols for crash debugging.
    if !is_dylib && !is_ios && !is_tvos && !is_harmonyos && target.as_deref() != Some("android")
