...
- **v0.5.129** — HarmonyOS build.rs native-object pickup + first `.so` that dlopens on NEXT emulator (PR B.6 for #113). Bug surfaced when `hdc install` succeeded but `onCreate` threw `TypeError: Cannot read property run of undefined` — OHOS dynamic linker had rejected `libentry.so` with `Error relocating ... mi_malloc_aligned: symbol not found`. Root cause: `libmimalloc-sys`'s build.rs compiles its C sources to a loose `<hash>-static.o` (362 KB, 154 `mi_*` symbols including `mi_malloc_aligned`) and also creates a `libmimalloc.a` wrapper — but on the macOS→OHOS cross-build that wrapper ends up as a zero-member BSD-format archive (BSD `ar`'s `__.SYMDEF SORTED` layout; llvm-ar can't even enumerate it). Rust's staticlib "bundle native libs" path silently skips empty archives, so `libperry_runtime.a` ships with zero `mi_*` definitions. On macOS/Linux targets Perry links through `rustc`'s driver which honors `cargo:rustc-link-lib=static=mimalloc` and pulls the loose .o anyway; but Perry's `is_harmonyos` branch invokes `clang` directly and misses rustc's link directives entirely. Fix: walk `target/<variant>/<triple>/release/build/*/out/` recursively (cc-rs can nest under `c_src/mimalloc/v2/src/` etc.) and append every `.o` to the final link command. Triggers for `perry-auto-<hash>/` (auto-rebuild path) and plain `<triple>/` (manual cargo build). Over-inclusion is fine: `--gc-sections` (already enabled for ELF targets in B.1) dead-strips anything the final binary doesn't reference, so picking up unused C deps costs nothing. Result on the wearable emulator: `libentry.so` grows 8.6 → 8.8 MB, `mi_malloc_aligned` moves from `UND` to `0x1aaed8 FUNC GLOBAL`, and `bm install` + `aa start EntryAbility` both succeed. **First time Perry-emitted code runs in a HarmonyOS process.** Also in this patch (unrelated to the linker fix but discovered during validation): target API bumped 12 → 21 to match DevEco 6.0.1(21)'s primary SDK; `deviceTypes` and pack.info `deviceType` lists gained `"wearable"` (only emulator image available to unverified Huawei accounts — phone emulator is gated behind real-name verification); ArkTS shim's skill action flipped from legacy `action.system.home` to `ohos.want.action.home` matching DevEco 6 templates. Still open (deliberate cutoff before handing off to #113 reporter for physical-device validation): whether TS `console.log()` actually reaches `hdc hilog` — perry-runtime writes to `println!()` → fd 1, which OHOS does not pipe into hilog by default; likely need a stdout→hilog shim. Also: `requireNapi('entry')` resolved via `.d.ts` + `file:` dependency in the DevEco project's `oh-package.json5` — not yet via direct `import ... from 'libentry.so'` (which ArkTS's strict compiler rejects without a declaration). Doc under `crates/perry/src/commands/harmonyos_hap.rs:9-24` now lists all seven `PERRY_HARMONYOS_*` env vars after the B.4 split — was stale, still mentioned only three.
- **v0.5.128** — HarmonyOS DevEco 6.x + first working `.hap` on disk (PR B.5 for #113). First real-SDK integration — all prior work was against docs. Three findings drove patches: **(1) DevEco 6.x SDK layout** differs from 5.x — the SDK is bundled inside `/Applications/DevEco-Studio.app/Contents/sdk/default/openharmony/` instead of `~/Library/Huawei/Sdk/openharmony/<api>/`, with a single `default/` folder replacing the per-API-level numeric dir. `find_harmonyos_sdk()` gained two new layout arms in `normalize()` (`p/default/openharmony/native` + `p/openharmony/native`) and a new probe for `/Applications/DevEco-Studio.app/Contents/sdk` (macOS-specific, gated on `target_os = "macos"`). **(2) macOS-host link-args leak into cross-target output** — the linker-command chain had an `} else { if cfg!(target_os = "macos") || is_cross_macos { ... -framework Security -liconv -lobjc } }` branch that fired for HarmonyOS (not ios/tvos/watchos/android/linux/windows). First real cross-compile showed `ld.lld: error: unknown argument '-framework'` + half a dozen Apple framework refs. Added a dedicated `is_harmonyos` arm before `is_linux` with OHOS-appropriate libs: `-Wl,--allow-multiple-definition -lm -lpthread -ldl -lace_napi.z` (musl folds m/pthread/dl into libc.a so the `-l` flags are no-ops, harmless; `libace_napi.z.so` provides the NAPI externs that `ohos_napi.rs` imports — OHOS's convention is `<name>.z.so` and `-l` strips `lib`/`.so` but not `.z`, so the spelling is deliberate). Also added `!is_harmonyos` to the host-macOS strip guard — BSD `strip` chokes on aarch64 ELF with a noisy "non-object and non-archive" warning. **(3) ets-loader's bare-invocation path doesn't work** — on inspection, ets-loader is a ~15-env-var-driven Node/rollup pipeline that reads `aceModuleRoot`/`aceModuleBuild`/`aceModuleJsonPath`/`aceProfilePath`/`compileMode=moduleJson` plus a full DevEco `build-profile.json5`; without them it exits 0 and produces no `.abc`. Synthesizing all of that re-implements a chunk of hvigor. Pivoted to direct `es2abc` invocation (empirically tested: `es2abc --extension ts` accepts plain-TypeScript `.ets` files, only balks on `@Entry @Component struct` ArkUI decorators). For Phase 1 that's fine — we ship a single `EntryAbility.ets` with no decorators, no `struct`, no UI. Dropped `Index.ets`, dropped `windowStage.loadContent('pages/Index')` in EntryAbility, dropped the `pages` field in module.json5, dropped `main_pages.json`. Phase 1 UIAbility just runs `perryEntry.run()` in `onCreate`; window is blank, `console.log` output lands in `hdc hilog`. PR C (TS→ArkTS emitter) will reintroduce ets-loader when it emits real ArkUI with decorators. `compile_ets_to_abc()` now collects every `.ets` under `staging/ets/` into a single `es2abc --module --merge-abc --extension ts --output ets/modules.abc <inputs>` invocation (HAPs ship a single merged `modules.abc`, not per-file .abc), then deletes the source files. **First end-to-end run verified on DevEco 6.0.1(21)**: `perry compile hi.ts --target harmonyos` produces `libentry.so` (8.6 MB, ELF aarch64, `libace_napi.z.so` + `libc.so` in NEEDED, `.init_array` with 2 pointers — Rust init + our `register_module`) + `entry.unsigned.hap` (9 MB, 17 members, `ets/modules.abc` with `PANDA` magic header, module name "entry" + bundleName fallback through cleanly, no stray `.ets` source). Remaining work before `hdc install` can succeed: signing env vars (`PERRY_HARMONYOS_P12` + four friends) from DevEco's auto-signing flow, then confirm the target API version (currently `min=11,target=12`) is compatible with DevEco 6.x's API 21 SDK (we cross-compiled fine; install-time rejection is still possible).
